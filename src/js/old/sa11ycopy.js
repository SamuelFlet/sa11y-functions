...
import Sa11yCustomChecks from "./sa11y-custom-checks.js";
import { buildSa11yUI } from "./buildSa11y.js";
	...
	constructor(options) {
		...
		this.initialize = () => {
			...
			const checkRunPrevent = () => {
				...
				return doNotRun.trim().length > 0
					? document.querySelector(doNotRun)
					: false;
			};
		...
		this.mainToggle = () => {
			...
			document.onkeydown = (e) => {
				...
				if (
					isEscape &&
					document
						.getElementById("sa11y-panel")
						.classList.contains("sa11y-active")
				) {
					sa11yToggle.setAttribute("aria-expanded", "false");
					sa11yToggle.classList.remove("sa11y-on");
					sa11yToggle.click();
		...
		this.utilities = () => {
			...
			Sa11y.computeTextNodeWithImage = ($el) => {
			...
			Sa11y.computeAriaLabel = (el) => {
				...
				if (
					Array.from(el.children).filter((x) => x.matches("[aria-label]"))
						.length > 0
				) {
			...
			Sa11y.findVisibleParent = (element, property, value) => {
				let $el = element;
				while ($el !== null) {
		...
		this.settingPanelToggles = () => {
			...
			const $linksToggle = document.getElementById(
				"sa11y-links-advanced-toggle"
			);
		...
		this.detectPageChanges = () => {
			...
			if (option.detectSPArouting === true) {
				...
				const checkURL = Sa11y.debounce(async () => {
		...
		this.checkAll = async () => {
			...
			if (!rootTarget) {
				...
				$alertText.innerHTML = `${Lang.sprintf(
			...
			if (option.linksAdvancedPlugin === true) {
				...
				const linksAdvancedLi = document.getElementById(
					"sa11y-links-advanced-li"
				);
			...
			if (option.readabilityPlugin === true) {
				...
				const readabilityPanel = document.getElementById(
					"sa11y-readability-panel"
				);
		...
		this.resetAll = (restartPanel = true) => {
			...
			const resetClass = (el) => {
				el.forEach((x) => {
					document
						.querySelectorAll(`.${x}`)
						.forEach((y) => y.classList.remove(x));
				});
			};
			resetClass([
				"sa11y-error-border",
			...
			// Alert within panel.
			document
				.querySelector("#sa11y-panel-alert")
				.classList.remove("sa11y-active");
			...
			while (emptyPreview.firstChild)
				emptyPreview.removeChild(emptyPreview.firstChild);
		...
		this.updatePanel = () => {
			...
			if (Sa11y.errorCount > 0 && Sa11y.warningCount > 0) {
				...
				$status.innerHTML = `${Lang._(
					"ERRORS"
				)} <span class="sa11y-panel-count sa11y-margin-right">${
					Sa11y.errorCount
				}</span> ${Lang._("WARNINGS")} <span class="sa11y-panel-count">${
					Sa11y.warningCount
				}</span>`;
				...
				$status.innerHTML = `${Lang._(
					"ERRORS"
				)} <span class="sa11y-panel-count">${Sa11y.errorCount}</span>`;
			} else if (Sa11y.warningCount > 0) {
				$panelContent.setAttribute("class", "sa11y-warnings");
				$status.innerHTML = `${Lang._(
					"WARNINGS"
				)} <span class="sa11y-panel-count">${Sa11y.warningCount}</span>`;
			} else {
		...
		this.buildPanel = () => {
			...
			const $settingsContent = document.getElementById(
				"sa11y-settings-content"
			);
			const $headingAnnotations = document.querySelectorAll(
				".sa11y-heading-label"
			);
		...
		this.skipToIssue = () => {
			...
			const scrollPosition = ($el) => {
				...
				if (offsetTopPosition === 0) {
					const visiblePosition = Sa11y.findVisibleParent(
						$el,
						"display",
						"none"
					);
					...
					if (visiblePosition) {
						...
						if (prevSibling) {
							return Sa11y.offsetTop(prevSibling).top - 150;
						}
						return Sa11y.offsetTop(parentNode).top - 150;
					}
				}
				removeAlert();
				return Sa11y.offsetTop($el).top - 150;
			};
		...
		this.findElements = () => {
			...
			const containerExclusions = Array.from(
				document.querySelectorAll(this.containerIgnore)
			);
			const readabilityExclusions = Array.from(
				document.querySelectorAll(this.readabilityIgnore)
			);

			// Contrast
			const $findcontrast = Array.from(container.querySelectorAll("*"));
			const excludeContrast = Array.from(
				container.querySelectorAll(this.contrastIgnore)
			);
			Sa11y.$contrast = $findcontrast.filter(
				($el) => !excludeContrast.includes($el)
			);
			...
			Sa11y.$readability = $findreadability.filter(
			...
			const excludeHeadings = Array.from(
				container.querySelectorAll(this.headerIgnore)
			);
			...
			const excludelinks = Array.from(
				container.querySelectorAll(this.linkIgnore)
			);
			...
			Sa11y.$inputs = $findinputs.filter(
				($el) =>
					!containerExclusions.includes($el) && !Sa11y.isElementHidden($el)
			);

			// Images
			const images = Array.from(container.querySelectorAll("img"));
			const excludeimages = Array.from(
				container.querySelectorAll(this.imageIgnore)
			);
			...
			Sa11y.$iframes = $findiframes.filter(
				($el) => !containerExclusions.includes($el)
			);
			Sa11y.$videos = Sa11y.$iframes.filter(($el) =>
				$el.matches(option.videoContent)
			);
			Sa11y.$audio = Sa11y.$iframes.filter(($el) =>
				$el.matches(option.audioContent)
			);
			Sa11y.$dataviz = Sa11y.$iframes.filter(($el) =>
				$el.matches(option.dataVizContent)
			);
			Sa11y.$twitter = Sa11y.$iframes.filter(($el) =>
				$el.matches(option.twitterContent)
			);
			Sa11y.$embeddedContent = Sa11y.$iframes.filter(
			...
			Sa11y.$strongitalics = $findstrongitalics.filter(
				($el) => !containerExclusions.includes($el)
			);
			...
			const $findbadDevLinks = option.linksToFlag
				...
				: [];
			Sa11y.$badDevLinks = $findbadDevLinks.filter(
				($el) => !containerExclusions.includes($el)
			);

			const $findPDFs = Array.from(
				container.querySelectorAll("a[href$='.pdf']")
			);
			Sa11y.$checkPDF = $findPDFs.filter(
				($el) => !containerExclusions.includes($el)
			);
			...
			Sa11y.$tables = $findtables.filter(
				($el) => !containerExclusions.includes($el)
			);

			Sa11y.$lang = document.querySelector("html").getAttribute("lang");

			const $findblockquotes = Array.from(
				container.querySelectorAll("blockquote")
			);
			Sa11y.$blockquotes = $findblockquotes.filter(
				($el) => !containerExclusions.includes($el)
			);
			...
			Sa11y.$allCaps = $findallcaps.filter(
				($el) => !containerExclusions.includes($el)
			);
		};
		...
		//----------------------------------------------------------------------
		Sa11y.annotate = (type, content, inline = false) => {
			let message = content;
			...
			message = message
				...
				.replaceAll(
					/<\/a>/g,
					`<span class="sa11y-visually-hidden"> (${Lang._(
						"NEW_TAB"
					)})</span></a>`
				)
				.replaceAll(/{r}/g, 'class="sa11y-red-text"');
			message = Sa11y.escapeHTML(message);

			return `<div class=${inline ? "sa11y-instance-inline" : "sa11y-instance"}>
                <button data-sa11y-annotation type="button" aria-label="${[
									type,
								]}" class="sa11y-btn sa11y-${CSSName[type]}-btn${
				inline ? "-text" : ""
			}" data-tippy-content="<div lang='${Lang._(
				"LANG_CODE"
		...
		//----------------------------------------------------------------------
		Sa11y.annotateBanner = (type, content) => {
			let message = content;
		...
		this.checkHeaders = () => {
			...
			Sa11y.$h.forEach(($el, i) => {
				const text = Sa11y.computeTextNodeWithImage($el);
				const htext = Sa11y.sanitizeForHTML(text);
				let level;
				...
				let ignoreArray = [];
				if (option.outlineIgnore) {
					ignoreArray = Array.from(
						document.querySelectorAll(this.outlineIgnore)
					);
				}
				...
				if (!ignoreArray.includes($el)) {
					...
					if (error !== null && $el.closest("a") !== null) {
						...
						$el
							...
							.insertAdjacentHTML(
								"afterend",
								Sa11y.annotate(ERROR, error, true)
							);
						document
						...
						$el.classList.add("sa11y-error-border");
						$el.insertAdjacentHTML("beforebegin", Sa11y.annotate(ERROR, error));
						document
							.querySelector("#sa11y-outline-list")
							.insertAdjacentHTML("beforeend", liError);
						...
						$el
							.closest("a")
							.insertAdjacentHTML("afterend", Sa11y.annotate(WARNING, warning));
						document
							.querySelector("#sa11y-outline-list")
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(WARNING, warning)
						);
						document
							.querySelector("#sa11y-outline-list")
							.insertAdjacentHTML("beforeend", liWarning);
			...
			if (Sa11y.$h1.length === 0) {
				...
				Sa11y.panel.insertAdjacentHTML(
					"afterend",
					Sa11y.annotateBanner(ERROR, Lang._("HEADING_MISSING_ONE"))
				);
			}
		};
		...
		this.checkLinkText = () => {
			...
			Sa11y.$links.forEach((el) => {
				let linkText = Sa11y.computeAriaLabel(el);
				...
				if (linkText === "noAria") {
					...
					if ($img) {
						...
						const imgText = Sa11y.computeAriaLabel($img);
						if (imgText !== "noAria") {
							linkText += imgText;
						} else {
				...
				const error = containsLinkTextStopWords(
					Sa11y.fnIgnore(el, option.linkIgnoreSpan)
				...
				if (el.querySelectorAll("img").length) {
					...
					if (el && hasTitle) {
						...
						el.insertAdjacentHTML(
							"afterend",
							Sa11y.annotate(ERROR, Lang._("LINK_EMPTY_LINK_NO_LABEL"), true)
						);
					} else {
						...
						el.insertAdjacentHTML(
							"afterend",
							Sa11y.annotate(ERROR, Lang._("LINK_EMPTY"), true)
						);
					}
					...
					if (
						...
						if (option.showGoodLinkButton === true) {
							el.insertAdjacentHTML(
								"beforebegin",
								Sa11y.annotate(GOOD, Lang.sprintf("LINK_LABEL", linkText), true)
							);
						}
					} else if (
						...
						el.insertAdjacentHTML(
							...
							Sa11y.annotate(
								ERROR,
								Lang.sprintf("LINK_STOPWORD", error[0]),
								true
							)
						);
					}
					...
					if (linkText.length > 40) {
						...
						el.insertAdjacentHTML(
							"afterend",
							Sa11y.annotate(WARNING, Lang._("LINK_URL"), true)
						);
					}
				} else if (
					hasAriaLabelledBy ||
        ...
        `);
				...
				if (
					...
					el.insertAdjacentHTML(
						"afterend",
						Sa11y.annotate(WARNING, Lang._("NEW_TAB_WARNING"), true)
					);
				}
				...
				if (fileTypeMatch && !containsFileTypePhrases) {
					...
					el.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(WARNING, Lang._("FILE_TYPE_WARNING"), true)
					);
				}
			});
		};
					...
					if ($el.closest("a[href]")) {
						if (
							...
							$el
								...
								.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(ERROR, Lang._("MISSING_ALT_LINK_MESSAGE"))
								);
						}
					} else {
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(ERROR, Lang._("MISSING_ALT_MESSAGE"))
						);
					}
				} else {
					// If alt attribute is present, further tests are done.
					const altText = Sa11y.sanitizeForHTML(alt); // Prevent tooltip from breaking.
					...
					if (error[0] !== null && $el.closest("a[href]")) {
						...
						if (
							...
							$el
								...
								.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(ERROR, Lang._("LINK_IMAGE_ARIA_HIDDEN"))
								);
						} else if (
							Sa11y.fnIgnore(
								$el.closest("a[href]"),
							...
							$el
								...
								.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(ERROR, Lang._("LINK_IMAGE_NO_ALT_TEXT"))
								);
						} else {
							$el
								.closest("a[href]")
								.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(GOOD, Lang._("LINK_IMAGE_HAS_TEXT"))
								);
						}
						...
						if ($el.closest("figure")) {
							...
							if (
								...
								$el.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(WARNING, Lang._("IMAGE_FIGURE_DECORATIVE"))
								);
							}
						} else {
							$el.classList.add("sa11y-warning-border");
							$el.insertAdjacentHTML(
								"beforebegin",
								Sa11y.annotate(WARNING, Lang._("IMAGE_DECORATIVE"))
							);
						}
						...
						if ($el.closest("figure")) {
							...
							if (
								...
								$el.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(GOOD, Lang.sprintf("IMAGE_PASS", altText))
								);
							}
						} else {
		...
		// ============================================================
		this.checkLabels = () => {
			Sa11y.$inputs.forEach((el) => {
				let ariaLabel = Sa11y.computeAriaLabel(el);
				...
				if (
					...
					el.insertAdjacentHTML(
						"afterend",
						Sa11y.annotate(WARNING, Lang._("LABELS_INPUT_RESET_MESSAGE"), true)
					);
				} else if (
					...
					el.insertAdjacentHTML(
						"afterend",
						Sa11y.annotate(ERROR, Lang._("LABELS_MISSING_LABEL_MESSAGE"), true)
					);
				}
			});
		};
		...
		this.checkEmbeddedContent = () => {
			...
			if (option.embeddedContentAudio === true) {
				Sa11y.$audio.forEach(($el) => {
					...
					$el.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(WARNING, Lang._("EMBED_AUDIO"))
					);
				});
			}
			...
			if (option.embeddedContentVideo === true) {
				Sa11y.$videos.forEach(($el) => {
					...
					if ($el.tagName === "VIDEO" && track.length) {
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(WARNING, Lang._("EMBED_VIDEO"))
						);
					}
				});
			}
			...
			if (option.embeddedContentDataViz === true) {
				Sa11y.$dataviz.forEach(($el) => {
					...
					$el.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(WARNING, Lang._("EMBED_DATA_VIZ"))
					);
				});
			}
			...
			if (option.embeddedContentTwitter === true) {
				Sa11y.$twitter.forEach(($el) => {
					...
					if (tweets.length > 3) {
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(WARNING, Lang._("EMBED_TWITTER"))
						);
					}
				});
			}
					...
					if (
						...
						if (
							...
							if ($el.getAttribute("aria-labelledby") === null) {
								...
								$el.insertAdjacentHTML(
									"beforebegin",
									Sa11y.annotate(ERROR, Lang._("EMBED_MISSING_TITLE"))
								);
							}
						}
					} else {
			...
			if (option.embeddedContentGeneral === true) {
				Sa11y.$embeddedContent.forEach(($el) => {
					if (
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(WARNING, Lang._("EMBED_GENERAL_WARNING"))
						);
					}
				});
			}
		...
		this.checkQA = () => {
			...
			if (option.badLinksQA === true) {
				Sa11y.$badDevLinks.forEach(($el) => {
					...
					$el.insertAdjacentHTML(
						"afterend",
						Sa11y.annotate(ERROR, Lang.sprintf("QA_BAD_LINK", $el), true)
					);
				});
			}
			...
			if (option.strongItalicsQA === true) {
				Sa11y.$strongitalics.forEach(($el) => {
					...
					if (strongItalicsText > 400) {
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(WARNING, Lang._("QA_BAD_ITALICS"))
						);
					}
				});
			}
			...
			if (option.pdfQA === true) {
				Sa11y.$checkPDF.forEach(($el, i) => {
					...
					if ($el && i === 0) {
						$el.insertAdjacentHTML(
							"afterend",
							Sa11y.annotate(WARNING, Lang.sprintf("QA_PDF", pdfCount), true)
						);
			...
			if (option.langQA === true) {
				if (!Sa11y.$lang || Sa11y.$lang.length < 2) {
					Sa11y.panel.insertAdjacentHTML(
						"afterend",
						Sa11y.annotateBanner(ERROR, Lang._("QA_PAGE_LANGUAGE"))
					);
				}
			}
			...
			if (option.tablesQA === true) {
				Sa11y.$tables.forEach(($el) => {
					...
					const findHeadingTags = $el.querySelectorAll(
						"h1, h2, h3, h4, h5, h6"
					);
					if (findTHeaders.length === 0) {
						...
						$el.insertAdjacentHTML(
							"beforebegin",
							Sa11y.annotate(ERROR, Lang._("TABLES_MISSING_HEADINGS"))
						);
					}
					if (findHeadingTags.length > 0) {
						findHeadingTags.forEach(($a) => {
							...
							$a.insertAdjacentHTML(
								"beforebegin",
								Sa11y.annotate(ERROR, Lang._("TABLES_SEMANTIC_HEADING"))
							);
						});
					}
					findTHeaders.forEach(($b) => {
						if ($b.textContent.trim().length === 0) {
							...
							$b.innerHTML = Sa11y.annotate(
								ERROR,
								Lang._("TABLES_EMPTY_HEADING")
							);
						}
					});
				});
			...
			if (option.allCapsQA === true) {
				...
				const $warningUppercase = Sa11y.root.querySelectorAll(
					".sa11y-warning-uppercase"
				);
				$warningUppercase.forEach(($el) => {
					$el.insertAdjacentHTML(
						"afterend",
						Sa11y.annotate(WARNING, Lang._("QA_UPPERCASE_WARNING"), true)
					);
				});
			}
				...
				ids.forEach(($el) => {
					...
					if (id) {
						if (allIds[id] === undefined) {
							...
							$el.insertAdjacentHTML(
								"afterend",
								Sa11y.annotate(ERROR, Lang.sprintf("QA_DUPLICATE_ID", id), true)
							);
						}
					}
				});
			...
			if (option.underlinedTextQA === true) {
				...
				underline.forEach(($el) => {
					...
					$el.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(WARNING, Lang._("QA_TEXT_UNDERLINE_WARNING"), true)
					);
				});
				const computed = Array.from(
					Sa11y.root.querySelectorAll(
			...
			if (option.pageTitleQA === true) {
				...
				if (!$title || $title.textContent.trim().length === 0) {
					Sa11y.panel.insertAdjacentHTML(
						"afterend",
						Sa11y.annotateBanner(ERROR, Lang._("QA_PAGE_TITLE"))
					);
				}
			}
		};
		...
		this.checkContrast = () => {
				...
				check() {
					...
					for (let i = 0; i < elements.length; i++) {
						...
						if (contrast) {
							...
							if (htmlTag === "SVG") {
								ratio =
									Math.round(contrast.contrastRatio(fill, background) * 100) /
									100;
								if (ratio < 3) {
									error = {
										elem,
								...
								if (background === "image") {
									...
									ratio =
										Math.round(
											contrast.contrastRatio(color, background) * 100
										) / 100;
									if (
										pointSize >= 18 ||
										(pointSize >= 14 && fontWeight >= 700)
									) {
										if (ratio < 3) {
											error = {
												elem,
			...
			contrastErrors.errors.forEach((item) => {
				...
				const nodetext = Sa11y.fnIgnore(clone, "script").textContent;
				if (name.tagName === "INPUT") {
					name.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(ERROR, Lang.sprintf("CONTRAST_INPUT_ERROR", cratio))
					);
				} else {
					name.insertAdjacentHTML(
						"beforebegin",
						Sa11y.annotate(
							ERROR,
							Lang.sprintf("CONTRAST_ERROR", cratio, nodetext)
						)
					);
				}
			});
			...
			contrastErrors.warnings.forEach((item) => {
				...
				name.insertAdjacentHTML(
					"beforebegin",
					Sa11y.annotate(WARNING, Lang.sprintf("CONTRAST_WARNING", nodetext))
				);
			});
		};
		// ============================================================
		...
		this.checkReadability = () => {
			...
			const $readabilityinfo = document.getElementById(
				"sa11y-readability-info"
			);
